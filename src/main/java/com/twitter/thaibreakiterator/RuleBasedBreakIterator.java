...
public class RuleBasedBreakIterator extends BreakIterator {
   ...
   * provided.
   */
  public RuleBasedBreakIterator(InputStream in) throws IOException, MissingResourceException {
    readTables(in);
  }
   ...
   * </pre>
   */
  protected void readTables(InputStream in)
  throws IOException, MissingResourceException {

    byte[] buffer = readFile(in);
  ...
  protected byte[] readFile(final InputStream is)
  throws IOException, MissingResourceException {
    ...
    if (is.read(buf) != len) {
      throw new MissingResourceException("Wrong header length", getClass().getSimpleName(), "");
    }

    /* Validate the magic number. */
    for (int i = 0; i < LABEL_LENGTH; i++, offset++) {
      if (buf[offset] != LABEL[offset]) {
        throw new MissingResourceException("Wrong magic number", getClass().getSimpleName(), "");
      }
    }

    /* Validate the version number. */
    if (buf[offset] != supportedVersion) {
      throw new MissingResourceException("Unsupported version(" + buf[offset] + ")", getClass().getSimpleName(), "");
    }
    ...
    buf = new byte[len];
    if (is.read(buf) != len) {
      throw new MissingResourceException("Wrong data length", getClass().getSimpleName(), "");
    }
